{- Epistemic model checker by Jan van Eijck. -}
...
isTrueAt
 m@(Mo worlds agents val acc points) w (Box ag f) = let
    r = rel ag m
    b = bl r w
  in
...
sexample =  [(P i, Box a (Info i)) | i <- [0..3 ]]
             ++
            [(Q i, Box b (Info i)) | i <- [0..3 ]]
             ++
            [(R i, Box c (Info i)) | i <- [0..3 ]]
...
ignA = Conj [Ng (Box a allBeer), Ng (Box a (Ng allBeer))]
ignB = Conj [Ng (Box b allBeer), Ng (Box b (Ng allBeer))]
ignC = Conj [Ng (Box c allBeer), Ng (Box c (Ng allBeer))]

knowC, knowC' :: Form  (Bool,Bool,Bool)
knowC  = Box c allBeer
knowC' = Box c (Ng allBeer)
...
statement_1 =
  Conj [ Ng (Box b (Info p)) | p <- pairs ]

statement_1e =
  Conj [ Info p `impl` Ng (Box b (Info p)) | p <- pairs ]

k_a_statement_1e = Box a statement_1e

statement_2 =
  Disj [ Box b (Info p) | p <- pairs ]

statement_2e =
  Conj [ Info p `impl` Box b (Info p) | p <- pairs ]

statement_3 =
  Disj [ Box a (Info p) | p <- pairs ]

statement_3e =
  Conj [ Info p `impl` Box a (Info p) | p <- pairs ]
...
kn_a = Disj [Box a (Disj [Info (True,x,y) | x <- bools,
                                           y <- bools]),
             Box a (Disj [Info (False,x,y) | x <- bools,
                                            y <- bools ])]

kn_b = Disj [Box b (Disj [Info (x,True,y) | x <- bools,
                                           y <- bools]),
             Box b (Disj [Info (x, False,y) | x <- bools,
                                             y <- bools ])]

kn_c = Disj [Box c (Disj [Info (x,y,True) | x <- bools,
                                           y <- bools]),
             Box c (Disj [Info (x,y,False) | x <- bools,
                                            y <- bools ])]
...
kn n i = Disj [Box (Ag i) (Disj [Info (tab1++[True]++tab2) |
                        tab1 <- bTables (i-1),
                        tab2 <- bTables (n-i) ]),
               Box (Ag i) (Disj [Info (tab1++[False]++tab2) |
                        tab1 <- bTables (i-1),
                        tab2 <- bTables (n-i) ])]
